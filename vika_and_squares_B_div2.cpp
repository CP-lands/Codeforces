...
void Anivia_kid(){
	...
	vector<int> set_min;
	for(int i = 0; i < n; i++)
		if(v[i] == minn) set_min.push_back(i);
	set_min.push_back(n + set_min[0]);
	ll ans = INT_MIN;

	for(int i = 0; i < set_min.size() - 1; i++){
		if(ans < set_min[i + 1] - set_min[i]) ans = set_min[i + 1] - set_min[i] - 1;
	}
	cout<<(ll)(minn * n + ans)<<endl;
